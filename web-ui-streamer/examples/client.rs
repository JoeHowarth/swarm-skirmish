...
//! stdout mutex management. Rather it's intended to show an example of working
//! with axum's websocket server and how the client-side and server-side code
...
const N_CLIENTS: usize = 10000; // set to desired number
...
async fn main() {
    ...
    println!(
        "Total time taken {:#?} with {N_CLIENTS} concurrent clients, should \
         be about 6.45 seconds.",
        end_time - start_time
    );
}
...
async fn spawn_client(who: usize) {
    let ws_stream = match connect_async(SERVER).await {
        Ok((stream, response)) => {
            ...
            // This will be the HTTP response, same as with server this is the
            // last moment we can still access HTTP stuff.
    ...
    let mut send_task = tokio::spawn(async move {
        for i in 1..30 {
            ...
            if sender
                ...
                .is_err()
            {
                // just as with server, if send fails there is nothing we can do
                // but exit.
                return;
            }
...
/// Function to handle messages we get (with a slight twist that Frame variant
/// is visible since we are working with the underlying tungstenite library
/// directly without axum here).
fn process_message(msg: Message, who: usize) -> ControlFlow<(), ()> {
    match msg {
        ...
        // Just as with axum server, the underlying tungstenite websocket
        // library will handle Ping for you automagically by replying
        // with Pong and copying the v according to spec. But if you
